...
class vec 
   ...
   vec rotate(float a, vec U) {setTo(R(this,a,U)); return this;} // rotate this V by a around U, assuming|U|==1 and V*U==0
...
vec R(vec V, float a, vec U) {vec W=cross(U,V); return V(cos(a),V,sin(a),W);} // rotate V by a around U, assuming|U|==1 and V*U==0
vec LPM(vec U, float t, vec V) // log-polar morph between U and V
  {
  vec B=U(cross(U,V)); 
  vec N=cross(B,U); 
  float a = angle(U,V);
  vec W = V(cos(a*t),U,sin(a*t),N);
  float m = n(V)/n(U);
  return V(pow(m,t),W);
  } 
